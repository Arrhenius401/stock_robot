...
执行需要工具调用、代码修改或多步骤分析的任务时，在 `commentary` 中主动提供简短、可审阅的过程摘要：

- 开始时说明目标与准备采取的方向。
- 工作超过约 60 秒时，更新当前进展、关键发现和下一步。
- 完成前说明已做的验证及结果。
- 摘要应聚焦事实、假设、决策和行动，不展示逐字内部推理或冗长思维链。
- 对简单问答或无需工具的任务，不必强制发送过程摘要。
- 优先使用“正在检查……”“发现……因此准备……”“已验证……”这类简短表述。
