...
除纯信息问答外，所有任务必须在 `commentary` 中留下可审计的外显决策记录：

1. 动手前：说明目标、已知事实、拟定方案，以及为何选此方案。
2. 每个关键决策：说明候选方案、取舍理由和最终选择。
3. 修改文件、执行命令或调用外部工具前：说明对象、目的与预期结果。
4. 发现异常、假设被推翻或方案改变时：说明新证据及调整原因。
5. 完成时：列出实际改动、验证命令及其结果、仍未验证的风险。

禁止只写“正在处理”“已完成”等无信息状态；应引用具体文件、命令输出、数据或测试结果。
不展示逐字内部推理，但必须充分说明可复核的事实、判断依据、决策与行动。
