...
- 真实启动命令为 `./.venv/Scripts/stock-robot.exe run --host 127.0.0.1 --port 8765`，不是 `stock-robot api`。前端改动必须在真实浏览器中验收，不能只交付 Mock 或静态测试。
- 效果图只确认布局方向；实现时必须读取并保留现有 CSS 变量、卡片、侧栏和控件风格，避免把局部报告改版做成另一套产品界面。
- 标的历史表现仅展示当前 ETF/股票自身数据；“同池策略参考”是池级轮动策略，文案、指标、成交记录和图例都要明确这一点。
- 回测短区间应从已完成的长区间缓存切片、归一化并重算指标，不能因切换区间重复运行策略。浏览器会话缓存键包含标的、快照日期和区间。
- 图表把净值转成累计收益率后绘制，保留基准切换；验收包含横轴日期、纵轴收益率、图例、等宽且位于图表下方的区间按钮，以及悬浮时两条曲线的同日数据。
- Windows 系统临时目录不可写时，Pytest 用 `--basetemp=tmp/pytest-run`，完成后删除临时目录。全量测试异常必须报告具体失败，不能把超时或空输出当作通过。
